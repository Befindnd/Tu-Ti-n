...
function describeRewards(n) {
  ...
  if (n.dan_duoc) {
    ...
    t.push(`${e?.emoji || "⚗️"} ${e?.ten || n.dan_duoc}${e?.limited ? " 💎 *(Limited)*" : ""} ×1`);
  }
  if (n.linh_can) {
...
async function applyGiftcodeRewards(n, t, e) {
  ...
  reg("xem_dao_thuong", ["xdt", "daothuong", "dt_xem"], async (n) => {
    const t = n.author.id,
      e = n.mentions.users.first(),
      h = e ? e.id : t,
      i = e ? e.username : n.author.username,
      a = await getPlayer(t);
    if (!a) return n.reply({ embeds: [errE("Dùng `-bat_dau`!")] });
    let o = a;
    if (e && e.id !== t && ((o = await getPlayer(h)), !o))
      return n.reply({ embeds: [errE(`**${i}** chưa tu tiên!`)] });
    const c = Math.min(3, Math.max(0, o.dao_thuong || 0)),
      _ = tinhCS(o),
      u = Number(o.dao_thuong_at || 0),
      r = _.hp_max > 0 ? Math.round((Number(o.hp) / _.hp_max) * 100) : 100,
      s = c > 0 && u > 0 ? Math.max(0, AUTO_HEAL_MS - (Date.now() - u)) : 0,
      l =
        (Math.floor(s / 36e5),
        Math.ceil((s % 36e5) / 6e4),
        0 === c ? 3066993 : 1 === c ? 15844367 : 2 === c ? 15105570 : 15158332),
      m = ["✅", CE("dt_nhe","🟡"), CE("dt_trung","🟠"), CE("dt_nang","🔴")][c];
    let g = "";
    g =
      0 === c
        ? `✅ **Thần thể lành mạnh hoàn toàn!**\n*Không có đạo thương — tu luyện không bị ảnh hưởng.*\n\n${CE("tip_icon","💡")} Đạo thương xuất hiện khi:\n• Thua PVP với HP rất thấp\n• Thua Boss trọng thương\n• Bị Ám Vệ lục soát liên tục`
        : `**${DT_TEN[c]}**\n*${DT_HIEU[c]}*`;
    const d = [];
    if (c > 0) {
      const n = 1 === c ? "-15%" : 2 === c ? "-30%" : "-50%",
        t = 1 === c ? "Không" : 2 === c ? "-10%" : "-20%",
        e = 1 === c ? "-30%" : 2 === c ? "-55%" : "-70%";
      (d.push({
        name: "📉 Ảnh Hưởng Chiến Lực",
        value: `${CE("tuatk", "⚔️")} Công Lực: **${n}**\n${CE("tudef", "🛡️")} Thủ Lực: **${t}**\n📖 Tu Vi: **${e}**`,
        inline: !0,
      }),
        d.push({
          name: `${CE("tuhp", "💜")} Linh Lực`,
          value: `**${fmt(Number(o.hp))} / ${fmt(_.hp_max)}**\n*(${r}%)*`,
          inline: !0,
        }));
      let h = "";
      for (let n = c; n > 0; n--) {
        const t = (c - n + 1) * AUTO_HEAL_MS - (Date.now() - u);
        const dtEmoji = ["", CE("dt_nhe","🟡"), CE("dt_trung","🟠"), CE("dt_nang","🔴")][n];
        if (t <= 0) h += `${dtEmoji} Cấp ${n} → ✅ Lành: **Sắp tự hồi...**\n`;
        else {
          const healUnix = Math.floor((u + (c - n + 1) * AUTO_HEAL_MS) / 1000);
          h += `${dtEmoji} Cấp ${n} → hồi <t:${healUnix}:R>\n`;
        }
      }
      d.push({
        name: "⏰ Lịch Tự Hồi Phục *(mỗi cấp cần 24h)*",
        value: h.trim() || "Sắp tự hồi...",
        inline: !1,
      });
      const i = "duoc_su" === o.nghe;
      d.push({
        name: "🩺 Cách Chữa",
        value: `• \`-chua_thuong\` tự chữa: **${fmt(i ? Math.floor(0.5 * PHI_TU_CHUA[c]) : PHI_TU_CHUA[c])}** ${CE("tult", "💠")} *(${i ? "Dược Sư -50%" : "CD 5h"})*\n• Nhờ **💉 Dược Sư** chữa: **${fmt(PHI_DUOC_SU[c])}** ${CE("tult", "💠")} *(CD riêng 45ph)*\n• Tự hồi phục: giảm 1 cấp mỗi **24h**`,
        inline: !1,
      });
    } else
      (d.push({
        name: `${CE("tuhp", "💜")} Linh Lực`,
        value: `**${fmt(Number(o.hp))} / ${fmt(_.hp_max)}** *(${r}%)*`,
        inline: !0,
      }),
        d.push({
          name: "⚔️ Chiến Lực",
          value: `${CE("tuatk", "⚔️")} Công: **${fmt(_.atk)}**  ·  ${CE("tudef", "🛡️")} Thủ: **${fmt(_.def)}**`,
          inline: !0,
        }));
    return n.reply({
      embeds: [
        new EmbedBuilder()
          .setTitle(`${m} Đạo Thương — ${i}`)
          .setColor(l)
          .setThumbnail(e ? e.displayAvatarURL() : n.author.displayAvatarURL())
          .setDescription(g)
          .addFields(d)
          .setFooter({ text: `Cấp Đạo Thương: ${c}/3 | Tự phục hồi 1 cấp mỗi 24h` }),
      ],
    });
  });
