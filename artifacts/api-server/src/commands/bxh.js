...
reg("canh_gioi", ["cg", "canhgioi"], async (n, t) => {
    const e = Math.max(1, parseInt(t[0]) || 1),
      h = Math.ceil(CANH_GIOI.length / 10),
      i = 10 * (e - 1),
      a = CANH_GIOI.slice(i, i + 10),
      o = new EmbedBuilder()
        .setTitle(`🌌 Bảng Cảnh Giới Tu Tiên — Trang ${e}/${h}`)
        .setColor(10181046)
        .setDescription(
          `*39 cảnh giới trên con đường đến cõi Tiên...*\n\n**Đại Cảnh Giới:**\nLuyện Khí → Trúc Cơ → Kết Đan → Nguyên Anh → Hóa Thần → Luyện Hư → Hợp Thể → Đại Thừa → Độ Kiếp → Tiên Nhân\n\n**Chênh Đại Cảnh Giới trong PvP:**\n+1 ĐCG: **+30% thắng** | +2 ĐCG: **+80% thắng** | +3 ĐCG: **Miểu Sát — từ chối chiến**\n\n${a.map((n) => `${CG_EMOJI(n.cap)} **Tầng ${n.cap}: ${n.ten}**\n› Tu Vi: **${fmt(n.exp_can)}** | ${CE("tuhp", "💜")} Linh Lực: ${fmt(n.linh_luc)}`).join("\n\n")}`,
        )
        .setFooter({ text: "Dùng -canh_gioi <trang> để xem thêm | Tổng 39 cảnh giới" });
    return n.reply({ embeds: [o] });
  });

  reg("bang_xep_hang", ["bxh", "xep_hang", "bangxephang", "xephang"], async (n, t) => {
    const e = (t[0] || "").toLowerCase();
    let h, i, a;
    "linh_thach" === e
      ? ((h = (
          await db(
            "SELECT username, linh_thach, canh_gioi FROM players ORDER BY linh_thach DESC LIMIT 10",
          )
        ).rows),
        (i = `${CE("tult", "💠")} Bảng Xếp Hạng — Giàu Nhất (Linh Thạch Thường)`),
        (a = (n, t) =>
          `**${t + 1}.** ${n.username} — **${fmt(n.linh_thach)}** ${CE("tult", "💠")} *(${getCG(n.canh_gioi).ten})*`))
      : "linh_thach_trung" === e || "trung" === e
        ? ((h = (
            await db(
              "SELECT username, linh_thach_trung, canh_gioi FROM players ORDER BY linh_thach_trung DESC LIMIT 10",
            )
          ).rows),
          (i = `${CE("tult_trung", "🔮")} Bảng Xếp Hạng — Giàu Nhất (Linh Thạch Trung)`),
          (a = (n, t) =>
            `**${t + 1}.** ${n.username} — **${fmt(n.linh_thach_trung||0)}** ${CE("tult_trung", "🔮")} *(${getCG(n.canh_gioi).ten})*`))
      : "linh_thach_cao" === e || "cao" === e
        ? ((h = (
            await db(
              "SELECT username, linh_thach_cao, canh_gioi FROM players ORDER BY linh_thach_cao DESC LIMIT 10",
            )
          ).rows),
          (i = `${CE("tult_cao", "💚")} Bảng Xếp Hạng — Giàu Nhất (Linh Thạch Cao)`),
          (a = (n, t) =>
            `**${t + 1}.** ${n.username} — **${fmt(n.linh_thach_cao||0)}** ${CE("tult_cao", "💚")} *(${getCG(n.canh_gioi).ten})*`))
      : "pvp" === e
        ? ((h = (
            await db(
              "SELECT username, pvp_wins, pvp_losses, canh_gioi FROM players ORDER BY pvp_wins DESC LIMIT 10",
            )
          ).rows),
          (i = `${CE("tuatk", "⚔️")} Bảng Xếp Hạng — PvP`),
          (a = (n, t) =>
            `**${t + 1}.** ${n.username} — **${n.pvp_wins}W/${n.pvp_losses}L** *(${getCG(n.canh_gioi).ten})*`))
        : ((h = (
            await db(
              "SELECT username, canh_gioi, exp, ngo_tinh, khi_van FROM players ORDER BY canh_gioi DESC, exp DESC LIMIT 10",
            )
          ).rows),
          (i = "🌌 Bảng Xếp Hạng — Cảnh Giới"),
          (a = (n, t) => {
            const e = getNgoTinh(n.ngo_tinh || 50);
            return `**${t + 1}.** ${n.username} — ${CG_EMOJI(n.canh_gioi)} **${getCG(n.canh_gioi).ten}** ${e.emoji}*(Ngộ ${n.ngo_tinh || 50})*`;
          }));
    const o = h.map((n, t) => a(n, t)).join("\n");
    return n.reply({
      embeds: [
        new EmbedBuilder()
          .setTitle(i)
          .setColor(16766720)
          .setDescription(o || "*Chưa có ai.*")
          .setFooter({ text: "-bxh | -bxh linh_thach | -bxh linh_thach_trung | -bxh linh_thach_cao | -bxh pvp" }),
      ],
    });
  });
