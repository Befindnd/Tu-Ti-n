...
async function xuLyBiCanhKetQua(n, t, e) {
  ...
  reg("hap_thu", ["ht", "hapthu"], async (n) => {
    const t = n.author.id,
      e = n.mentions.users.first();
    if (!e || e.id === t || e.bot)
      return n.reply({ embeds: [errE("Cú pháp: `-hap_thu @người_chơi`")] });
    const [h, i] = await Promise.all([getPlayer(t), getPlayer(e.id)]);
    if (!h) return n.reply({ embeds: [errE("Dùng `-bat_dau`!")] });
    if (!i) return n.reply({ embeds: [errE(`**${e.username}** chưa tu tiên!`)] });
    if ("ma_dao" !== h.cong_phap)
      return n.reply({ embeds: [errE("Chỉ dành cho **Huyết Ma Đại Pháp**!")] });
    const a = cdRemMin(h.hap_thu_cd, 45);
    if (a) return n.reply({ embeds: [warnE(`Hết CD ${cdTsMin(h.hap_thu_cd, 45)}.`)] });
    const o = Math.min(0.55, Math.max(0.1, 0.3 + 0.02 * (h.canh_gioi - i.canh_gioi)));
    if (
      (await db("UPDATE players SET hap_thu_cd=$1 WHERE user_id=$2", [Date.now(), t]),
      Math.random() < o)
    ) {
      const a = Math.floor(0.025 * Number(i.exp)),
        c = Math.floor(0.03 * Number(h.linh_thach));
      return (
        await db(
          "UPDATE players SET exp=exp+$1, linh_thach=GREATEST(0,linh_thach-$2), tam_ma=GREATEST(-100,tam_ma-20), nhan_qua=GREATEST(-100,nhan_qua-12), ma_khi=ma_khi+12 WHERE user_id=$3",
          [a, c, t],
        ),
        await db("UPDATE players SET exp=GREATEST(0,exp-$1) WHERE user_id=$2", [a, e.id]),
        n.reply({
          embeds: [
            new EmbedBuilder()
              .setTitle("🩸 Huyết Ma Hấp Thu Thành Công!")
              .setColor(9109504)
              .setDescription(
                `${CE("tutv", "📈")} +**${fmt(a)}** Tu Vi từ **${e.username}**!\n💸 Tiêu **${fmt(c)}** Linh Thạch để khai công pháp\n😈 Đạo Tâm -20 | Nghiệp Lực +12`,
              )
              .setFooter({ text: `Tỉ lệ: ${Math.round(100 * o)}% | CD: 45 phút` }),
          ],
        })
      );
    }
    const c = Math.floor(0.04 * Number(h.exp));
    return (
      await db(
        "UPDATE players SET exp=GREATEST(0,exp-$1), tam_ma=GREATEST(-100,tam_ma-5) WHERE user_id=$2",
        [c, t],
      ),
      n.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle("🩸 Huyết Ma Thất Bại!")
            .setColor(15158332)
            .setDescription(
              `*Thần thức của **${e.username}** kháng cự mạnh mẽ — phản thần thức gây tổn thương!*\n\n📉 Mất **${fmt(c)}** Tu Vi *(4% tích lũy)* | ${CE("tam_ma","😈")} Đạo Tâm -5`,
            )
            .setFooter({ text: `Tỉ lệ: ${Math.round(100 * o)}% | CD: 45 phút` }),
        ],
      })
