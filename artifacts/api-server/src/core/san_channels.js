'use strict';
/**
 * core/san_channels.js
 * Whitelist kênh Săn Linh Thú — theo TỪNG SERVER (guild) riêng biệt.
 * Nếu whitelist của 1 server rỗng hoặc tắt → Săn dùng được ở mọi kênh của server đó.
 * Lưu vào bot_settings key = 'san_channels' dưới dạng { [guildId]: { enabled, ids } }.
 *
 * Hoạt động độc lập với channel whitelist thông thường (allowed_channels):
 * - allowed_channels: giới hạn MỌI lệnh bot
 * - san_channels:     chỉ giới hạn riêng lệnh Săn Linh Thú
 */
...
async function load() {
  try {
    const res = await db(`SELECT value FROM bot_settings WHERE key=$1`, ['san_channels']);
    _byGuild.clear();
...
async function _save() {
  ...
  await db(
    ...
    ['san_channels', JSON.stringify(payload)],
  ).catch(() => {});
}
