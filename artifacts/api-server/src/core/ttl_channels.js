'use strict';
/**
 * core/ttl_channels.js
 * Whitelist kênh Tháp Thị Luyện (TTL) — theo TỪNG SERVER (guild) riêng biệt.
 * Nếu whitelist của 1 server rỗng hoặc tắt → TTL dùng được ở mọi kênh của server đó.
 * Lưu vào bot_settings key = 'ttl_channels' dưới dạng { [guildId]: { enabled, ids } }.
 *
 * Hoạt động độc lập với channel whitelist thông thường (allowed_channels):
 * - allowed_channels: giới hạn MỌI lệnh bot
 * - ttl_channels:     chỉ giới hạn riêng lệnh Tháp Thị Luyện
 */
...
async function load() {
  try {
    const res = await db(`SELECT value FROM bot_settings WHERE key=$1`, ['ttl_channels']);
    _byGuild.clear();
...
async function _save() {
  ...
  await db(
    ...
    ['ttl_channels', JSON.stringify(payload)],
  ).catch(() => {});
}
