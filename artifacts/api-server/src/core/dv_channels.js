...
/**
 * core/dv_channels.js
 * Whitelist kênh Đố Vui — theo TỪNG SERVER (guild) riêng biệt.
 * Nếu whitelist của 1 server rỗng hoặc tắt → Đố Vui dùng được ở mọi kênh của server đó.
 * Lưu vào bot_settings key = 'dv_channels' dưới dạng { [guildId]: { enabled, ids } }.
 *
 * Hoạt động độc lập với channel whitelist thông thường (allowed_channels):
 * - allowed_channels: giới hạn MỌI lệnh bot
 * - dv_channels:      chỉ giới hạn riêng lệnh Đố Vui
 */
...
async function load() {
  try {
    const res = await db(`SELECT value FROM bot_settings WHERE key=$1`, ['dv_channels']);
    _byGuild.clear();
...
async function _save() {
  ...
  await db(
    ...
    ['dv_channels', JSON.stringify(payload)],
  ).catch(() => {});
}
